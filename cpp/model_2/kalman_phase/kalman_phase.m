...
function [yhat,xhatfilt,xhatpred,P,Q] = kalman_phase(y,F,G,H,R1,R2,x0,Q0,backward)
%x0=[phase;freq_off];
K=size(y,1);
...
%xhatfilt(:,1)=xhatpred(:,1)+L*(rem(conj(y(1,:)')-yhat(:,1),pi));
Q(:,:,1)=P(:,:,1)-L*H*P(:,:,1);
% for i=2:length(y)
...
%     xhatfilt(:,i)=xhatpred(:,i)+L*(rem(conj(y(i,:)')-yhat(:,i),pi));
%     Q(:,:,i)=P(:,:,i)-L*H*P(:,:,i);
% end
% if backward
