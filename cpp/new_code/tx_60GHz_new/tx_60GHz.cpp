...
int UHD_SAFE_MAIN(int argc, char *argv[]){
    ...
    desc.add_options()
      ...
      ("nsamps", po::value<size_t>(&total_num_samps)->default_value(37028), "total number of samples to transmit")
      ...
      ("gain",po::value<float>(&gain)->default_value(3), "gain of transmitter(0-13) ")
    ...
    tx_stream->send(buffer,total_num_samps,md,60);
     ...
     md.start_of_burst = false;  //dev->set_time_now(uhd::time_spec_t(0.0));
    md.start_of_burst = true;
    md.end_of_burst = false;
    md.has_time_spec = false;
    //md.time_spec = uhd::time_spec_t(seconds_in_future);
