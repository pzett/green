...
void processing(short *data,int nStorage, int name){
 ...
 // Save data received to file
    string s1= "received.dat" ;
    std::ofstream ofs(s1, std::ifstream::out );
    ofs.write((char * ) data, nStorage*sizeof(short));
    ofs.close();
    std::cout << "Received sequence wrote into file. Start data processing...\n";
    ...
    int r=system("octave cppresults.m");
...
void detection(uint nDetect, int nStorage){
  ...
  while(1){
	...
	storage_short=new short[nStorage];
      ...
      if (count_trans == 10000){std::cout << "Ready to receive!\n";}
      if (power>10000&& count_trans>10000 && count<100){
	// If detection, keep the element
	std::cout<<"Transmission detected. Saving the data...\n";
	isDetected=true;
      }
      else{
	// Release the element
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
  ...
  desc.add_options()
    ("help", "help message")
    ("nsamps", po::value<size_t>(&total_num_samps)->default_value(45000), "total number of samples to receive")
    ("rxrate", po::value<double>(&rx_rate)->default_value(100e6/4), "rate of incoming samples")
    ("freq", po::value<double>(&freq)->default_value(4.9e9), "rf center frequency in Hz")
  ...
  // Set highest priiority for usrpT
    ...
    std::cout << "Priority of usrpT set to 99!\n"<< std::endl;
