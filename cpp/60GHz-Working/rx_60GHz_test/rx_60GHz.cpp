...
void detection(uint nDetect, int nStorage){
  ...
  while(1){
	...
	storage_short=new short[nStorage];
      ...
      if ( power>500 && count_trans>10000 && count<350){
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
  ...
  if (!(uhd::set_thread_priority_safe(1,true))) {
    ...
    desc.add_options()
      ("help", "help message")
      ("nsamps", po::value<size_t>(&total_num_samps)->default_value(11000), "total number of samples to receive")
      ...
      ("gain",po::value<float>(&gain)->default_value(12), "set the receiver gain (0-15)") 
