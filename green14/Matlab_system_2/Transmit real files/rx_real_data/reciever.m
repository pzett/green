...
if strcmp(decoding_method, 'Kalman_parametric')
    [data_received_equalized, channel_est, channel_est_pilot ,theta_est_filt] = Kalman_parametric( rx_data_reshaped, train_seq, rx_data_train, phi_hat, nr_car, nr_used_car, length_cpre, length_suf, Q, car_pattern, pilot_pattern, pilot, nr_OFDM_symbols,length_pilot,theta_est,length_past, length_fut);
end
...
if strcmp(decoding_method, 'Kalman_combined')
    [data_received_equalized_1, channel_est_1, channel_est_pilot_1 ,theta_est_filt_1] = Kalman_parametric( rx_data_reshaped, train_seq, rx_data_train, phi_hat, nr_car, nr_used_car, length_cpre, length_suf, Q, car_pattern, pilot_pattern, pilot, nr_OFDM_symbols,length_pilot,theta_est,length_past, length_fut);
    [data_received_equalized_2, channel_est_2, channel_est_pilot_2] = Kalman_lovers( rx_data_reshaped, train_seq, rx_data_train, phi_hat, nr_car, nr_used_car, length_cpre, length_suf, Q, car_pattern, pilot_pattern, pilot, nr_OFDM_symbols,length_pilot,gain_pilot_0);
    channel_est=abs(channel_est_2).*exp(angle(channel_est_1)*1i);
    [rx_data] = OFDM_inv(rx_data_reshaped,nr_car,length_cpre,length_suf);
    data_received=rx_data(car_pattern,:);
    data_received_equalized=data_received./channel_est;
end
figure(2)
