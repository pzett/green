...
void processing(short *data,int nStorage, int name){
  short *data_bin;
  int total_data=131072;//81600;//6800;// the total number of bits
  data_bin=new short[total_data];
  ...
  receiverSignalProcessing(data, nStorage, data_bin, total_data);
  //std::cout<<"Detected!\n";
  //exit(1);
  ...
  int a=system("octave image_transmition.m &");
  ...
  std::cout << "Data Received and Processed!\n";
   ...
   ofs1.write((char * ) data_bin, total_data*sizeof(short));
   ofs1.close();
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
  ...
  if (!(uhd::set_thread_priority_safe(1,true))) {
    ...
    desc.add_options()
      ("help", "help message")
      ("nsamps", po::value<size_t>(&total_num_samps)->default_value(185140), "total number of samples to receive")
