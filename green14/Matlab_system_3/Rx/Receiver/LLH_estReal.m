...
%rho=1/sqrt(2) for bpsk modulation.
    ...
    data_rx_train=rx_data_reshaped; %Because the first OFDM symbol is training
...
%     epsilon=1/(pi*nr_car)*(mod(angle(data_rx_train(1:nr_car/2)'*(R1*PHN_cos_delta*R1'+4*sigma_sqr*eye(nr_car/2))^-1*data_rx_train(nr_car/2+1:end))+pi,2*pi)+pi) %Offset frequence
...
%     epsilon=1e-3;
    E=diag(exp(1i*2*pi*epsilon*[1:nr_car])); %Diagonal matrix with the f_offset
