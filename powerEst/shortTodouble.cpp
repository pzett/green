...
int main () {
  int nElem=8;
  double data[]={0, 2.9, 3, 4, 5, 6, 7, -10};
  complex<double> *data2= (complex<double> *) data ;
  for (int i =0; i<nElem/2;i++){
  std::cout << "datas to be sent:" << data2[i]<<".." << '\n';
  }
  // std::cout <<"a size. "<< a.size()<< "\n";


  complex<short> out[nElem/2];
  compDoubleToCompShort(data2, nElem/2, out);

  //std::cout << "power= " << out.get(0, out.size()-1) << '\n';
  for (int i =0; i<nElem/2;i++){
  std::cout << "datas received:" << out[i]<<".." << '\n';
  }
}
*/
